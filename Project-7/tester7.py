...
REQUIRED_DEFNS = ['voterPrecinct', 'findPrecinctInfo', 'pollingPrecincts', 'voter_data']
...
class AllTests (unittest.TestCase):
	...
	def setUpClass():
		cur_dir = files_list(".")
		assert os.path.abspath("voters.csv") in cur_dir, '\n\nunable to locate voters.csv!\nmove file to the current directory and try again.'
		assert os.path.abspath("precincts_raw.csv") in cur_dir, '\n\nunable to locate precincts_raw.csv!\nmove file to the current directory and try again.'
	...
	############################################################################
	#voterPrecinct Tests
	def test_voterPrecinct_1(self):
		self.assertEqual(voterPrecinct("voters.csv", "itai tracee"), "Name: Itai Tracee Precinct: Kingstowne")
	def test_voterPrecinct_2(self):
		self.assertEqual(voterPrecinct("voters.csv", "OkEaNos ALoysius"), "Name: Okeanos Aloysius Precinct: Robinson")
	def test_voterPrecinct_3(self):
		self.assertEqual(voterPrecinct("voters.csv", "Mawar EALDWINE"), "Name: Mawar Ealdwine Precinct: Crestwood")
	def test_voterPrecinct_4(self):
		self.assertEqual(voterPrecinct("voters.csv", "SOROUSH inbal"), "Name: Soroush Inbal Precinct: Crestwood")

	
	#findPrecinctInfo Tests
	def test_findPrecinctInfo_1(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "SARATOGA"), "Saratoga Saratoga Elementary School 8111 Northumberland Rd Springfield")
	def test_findPrecinctInfo_2(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "KINGSTOWNE"), "Kingstowne Hayfield Secondary School 7630 Telegraph Rd Alexandria" )
	def test_findPrecinctInfo_3(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "GREENSPRING"), "Greenspring Greenspring Conference Center 7430 Spring Village Dr Springfield")
	def test_findPrecinctInfo_4(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "WEST SPRINGFIELD"), "West Springfield West Springfield Elementary School 6802 Deland Dr Springfield")
	def test_findPrecinctInfo_5(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "CHERRY RUN"), "Cherry Run Cherry Run Elementary School 9732 Ironmaster Dr Burke")
	def test_findPrecinctInfo_6(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "VALLEY"), "Valley Rolling Valley Elementary School 6703 Barnack Dr Springfield")
	def test_findPrecinctInfo_7(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "GARFIELD"), "Garfield Garfield Elementary School 7101 Old Keene Mill Rd Springfield" )
	def test_findPrecinctInfo_8(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "PARKWAY"), "Parkway Burke Community Church 9900 Old Keene Mill Rd Burke")
	def test_findPrecinctInfo_9(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "TERRA CENTRE"), "Terra Centre Terra Centre Elementary School 6000 Burke Centre Pky Burke")
	def test_findPrecinctInfo_10(self):
		self.assertEqual(findPrecinctInfo("precincts_raw.csv", "NOTHING"), "")

	
	#pollingPrecincts Tests
	def test_pollingPrecincts_1(self):
		d = {'springfield': ['saratoga', 'greenspring', 'west springfield', 'valley', 'garfield', 'keene mill', 'irving', 'crestwood', 'cardinal', 'burke', 10], 'alexandria': ['kingstowne', 'bush hill', 'cameron', 'bren mar', 4], 'burke': ['cherry run', 'parkway', 'terra centre', 'white oaks', 'burke centre', 'signal hill', 6], 'fairfax station': ['fairview', 1], 'fairfax': ['laurel', 'robinson', 2], 'annandale': ['brook hill', 1]}
		self.assertEqual(pollingPrecincts("precincts3.csv"), d)
        
	def test_pollingPrecincts_2(self):
		d = {'alexandria': ['kingstowne', 'bren mar', 2], 'springfield': ['crestwood', 'cardinal', 2], 'fairfax': ['robinson', 1], 'annandale': ['brook hill', 1]}
		self.assertEqual(pollingPrecincts("precincts1.csv"), d)
        
	def test_pollingPrecincts_3(self):
		d = {'springfield': ['valley', 'garfield', 'keene mill', 'irving', 'crestwood', 'cardinal', 'burke', 7], 'burke': ['parkway', 'terra centre', 'white oaks', 'burke centre', 'signal hill', 5], 'fairfax station': ['fairview', 1], 'alexandria': ['bush hill', 'cameron', 'bren mar', 3], 'fairfax': ['laurel', 'robinson', 2], 'annandale': ['brook hill', 1]}
		self.assertEqual(pollingPrecincts("precincts2.csv"), d)
		

	#voterData Tests
	def test_voter_data_1(self):
		result = [{'Full Name': 'Atticus Finch', 'Particpation Rate': 0.85, 'Zip Code': 20025}, {'Full Name': 'Marni Walters', 'Particpation Rate': 0.36, 'Zip Code': 97214}, {'Full Name': 'Bethany Bachan', 'Particpation Rate': 0.72, 'Zip Code': 0}]
		ls = ["Atticus Finch", ".85", "20025", "Marni Walters", ".36", "97214", "Bethany Bachan", ".72", "221155"]
		self.assertEqual(voter_data(ls), result)
	def test_voter_data_2(self):
		result = [{'Full Name': 'Bevin Borbel', 'Particpation Rate': 0.99, 'Zip Code': 20003}, {'Full Name': 'Carlos Cornell', 'Particpation Rate': 0.55, 'Zip Code': 0}, {'Full Name': 'Davida Davis', 'Particpation Rate': 0.89, 'Zip Code': 21855}, {'Full Name': 'Edwin Edwards', 'Particpation Rate': 0, 'Zip Code': 10045}, {'Full Name': 'Faisal Farrell', 'Particpation Rate': 0.84, 'Zip Code': 10424}]
		ls = ["Bevin Borbel", ".99", "20003", "Carlos Cornell", ".55", "219535","Davida Davis", ".89", "21855", "Edwin Edwards", "yes", "10045", "Faisal Farrell", ".84", "10424", "Gerri Mander", ".42"]
		self.assertEqual(voter_data(ls), result)
	def test_voter_data_3(self):
		result = []
		ls = ['Any Name']
		self.assertEqual(voter_data(ls), result)
